...
/-!
# P2: "The Dual Resilience Paradox" — Complete Formal Verification

All 4 core mathematical claims from the paper, verified in Lean 4.
Paper: "Hub Removal Improves Percolation While Enabling Threshold Cascades
        in Scale-Free Networks" (Cachero, 2026)
-/

/-!
## Claim 1: Cascade Firewall Condition (§2.5, Appendix A.1)

A node with degree k > 1/φ requires ≥ 2 active neighbors to activate.
Formally: k > 1/φ ⟹ k·φ > 1.
-/
theorem cascade_firewall (k : ℝ) (φ : ℝ)
...
/-!
## Claim 2: Molloy-Reed Monotonicity (§3.1, §4.1)
...
/-!
## Claim 3: Necessary Condition φ* ≥ √α/m (§4.2, Appendix A.2)

For BA networks: minimum hub degree k_min ≈ m/√α.
If k_min > 1/φ (hubs are firewalls), then φ > √α/m.
...
When k > 2⟨k⟩ AND k > 1/φ:
  ...
  (ii) k·φ > 1           (cascade firewall property)
...
|-------|--------------|-----------|--------|
| 1 | §2.5, A.1 | k > 1/φ ⟹ k·φ > 1 | ✅ |
| 2 | §3.1, §4.1 | κ₁ > κ₂ > 1 ⟹ 1/(κ₁-1) < 1/(κ₂-1) | ✅ |
| 3 | §4.2, A.2 | m/√α > 1/φ ⟹ φ > √α/m | ✅ |
| 4a | §5.1 | k ≥ 2, φ ∈ (0,1] ⟹ k² ≥ k·φ | ✅ |
