...
\textbf{Regime II ($0.15 < \varphi < 0.27$): Phase transition zone.} Hub removal shifts the network from subcritical to supercritical cascade dynamics. At $\varphi=0.22$, mean cascade size increases from $0.86\%$ [95\% CI: 0.43--1.30] to $23.1\%$ [95\% CI: 21.3--24.9] ($n=1{,}000$ trials: 50 networks $\times$ 20 seeds). This constitutes a genuine phase transition, confirmed by bimodal cascade size distributions (Fig.~\ref{fig:bimodal}): the probability of a global cascade ($C > 10\%$) rises from 0/1{,}000 before removal to 377/1{,}000 after removal ($P_{\mathrm{global}}=0.377$, Wilson 95\% CI: $[0.348, 0.406]$). Post-removal trials split between a subcritical mode ($C < 1\%$) and a supercritical mode ($C > 10\%$), with few intermediate values---the hallmark of a system straddling the critical point. The hub vulnerability experiment (Table~\ref{tab:hub_vuln}, $n=200$) yields consistent results ($0.28\% \to 18.7\%$) under a controlled single-network protocol.
...
Hub removal produces \textbf{compounding vulnerability}: both percolation and cascade metrics worsen simultaneously. The percolation threshold increases by $+347\%$ ($\pc$: $0.174 \to 0.776$, susceptibility-peak method), meaning the remaining network requires far more edge retention for connectivity. Simultaneously, the cascade window expands, enabling global cascades at previously safe operating points. This compounding---rather than a trade-off---is the central finding: hub removal does not exchange one type of resilience for another but degrades both.

As expected from model independence, $\DF_p$ is independent of cascade threshold $\varphi$, since percolation depends only on the static degree sequence. The susceptibility-peak method yields $\DF_p = +347\%$ ($\pc$: $0.174 \to 0.776$); the $S=0.5$ crossing method yields $+170\%$ ($\pc$: $0.331 \to 0.895$). Both confirm massive percolation degradation. We report susceptibility-peak values as the primary estimator.
...
Cascade amplification under hub removal is \emph{universal} across all four networks, with amplification factors ranging from $+1{,}327\%$ (Bitcoin OTC) to $+60{,}272\%$ (Wiki-Vote). The magnitude scales with $\kappa$: p2p-Gnutella ($\kappa=17.7$), which is closest to the BA model in both $\kappa$ and clustering ($C=0.011$), exhibits clean compounding vulnerability with both percolation ($\DF_p=+75\%$) and cascade metrics worsening simultaneously. Wiki-Vote ($\kappa=145.4$) shows an extreme version: $\DF_p=+594\%$ and cascade amplification of $+60{,}272\%$.

CA-HepPh ($C=0.622$) is instructive: despite $\kappa=130.9$, the susceptibility-peak method yields anomalous percolation results, while the $S=0.5$ method confirms $\DF_p=+58\%$. This highlights that high clustering can interact with the susceptibility-peak estimator in ways that require further investigation. The cascade amplification ($+7{,}340\%$) is nevertheless robust across estimators.
