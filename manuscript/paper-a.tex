...
\title{Compounding Vulnerability: Hub Removal Can Trigger Cascade Phase Transitions While Degrading Percolation Robustness in Barab\'asi--Albert Networks}
...
\begin{enumerate}
\item A systematic study of the \emph{joint} percolation-cascade phase structure under hub removal in BA networks, demonstrating that both stress models can worsen simultaneously at intermediate cascade thresholds---a phenomenon that neither stress model alone would predict.

\item A controlled experimental decomposition showing that hub-mediated cascade suppression is primarily \emph{dynamical} (threshold-based) rather than structural (connectivity-based), demonstrated by the ${\sim}95\%/{\sim}19\%/{\sim}0.3\%$ cascade split across hub vulnerability conditions on the same network topology.

\item A closed-form analytical derivation of the post-removal cascade branching factor $z_1(\varphi, \alpha, m)$ under the configuration-model approximation, providing a predictive expression for cascade window expansion under targeted hub removal.
\end{enumerate}
